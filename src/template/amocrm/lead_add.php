...
$leads[ 'request' ][ 'leads' ][ 'add' ][] = array(
			...
			'name' => $data[ 'name_lead' ] . ' ' . $data[ 'geo_location' ] . ' (' . $data[ 'page_place' ] . ')', #Имя сделки
			'pipeline_id' => 212412, #Id воронки
			'status_id' => 11360994, #Id этапа сделки
			'responsible_user_id' => $task_id_name[ 'Дарья Соколовская' ], #Ответственный пользователь
			'tags' => $data[ 'geo_location' ] . ', mossebo.studio, ' . $data[ 'geo_office' ] . ', ' . $data['utm_source'], #Теги

			'custom_fields' => array( # Дополнительные поля

				# Дополнительное поле 'Площадь помещения'.
				array(
					'id'=> $custom_fields[ 'Площадь помещения' ],
					'values' => array(
						array(
							'value'=> $data[ 'area' ]
						)
				...
				# Дополнительное поле 'Тип помещения'.
  			array(
  				'id'=>$custom_fields[ 'Тип помещения' ],
  				'values'=> array(
  					array(
				...
				# Дополнительное поле 'Тип ремонта'.
  			array(
  				'id'=>$custom_fields[ 'Тип ремонта' ],
  				'values'=> array(
  					array(
  						'value' => $data[ 'fix_type' ]
  					)
